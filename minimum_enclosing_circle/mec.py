...
def is_in_circle(p: Point, c: Point, r: float) -> bool:
    return np.linalg.norm(np.array(p) - np.array(c)) <= r + 1e-10

def circle_from(p1: Point, p2: Point, p3: Point = None) -> Tuple[Point, float]:
    if p3 is None:
        center = ((p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2)
        radius = np.linalg.norm(np.array(p1) - np.array(center))
    ...
    try:
        ...
        pts = [p1, p2, p3]
        i, j = np.unravel_index(np.argmax(dists), (3, 3))
        return circle_from(pts[i], pts[j])
...
def plot_circle(points: List[Point], center: Point, radius: float, out_path: str = None):
    xs, ys = zip(*points)
    fig = go.Figure()
    # Body
